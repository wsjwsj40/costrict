...
export interface LoggerOptions {
	/** Minimum output level (default: Debug in development, Info otherwise) */
	level?: LogLevel
	/** Whether to enable logging (default: true) */
	enabled?: boolean
	/** Custom timestamp function (default: locale string) */
	timeFn?: () => string
...
class ChannelLogger implements ILogger {
	...
	private log(level: LogLevel, tag: string, msg: unknown, ...args: unknown[]): void {
		...
		const line = `[${this.timeFn()}] [${tag}] ` + [msg, ...args].map((value) => this.safeToString(value)).join(" ")
