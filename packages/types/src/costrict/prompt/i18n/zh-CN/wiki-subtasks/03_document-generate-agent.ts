...
export const DOCUMENT_GENERATION_AGENT_TEMPLATE: SubtaskTemplateFactory = (workspace: string) => `# 技术文档生成
...
**长文档写入规则（强制）：**
- 不得用一次 \`write_to_file\` 调用生成整篇长文档；单次 \`content\` 控制在约 6000 字符以内
- 第一块使用 \`operation: "overwrite"\` 创建文件，后续内容按顺序使用 \`operation: "append"\` 追加
- 每块必须是连续的真实内容，不得使用“其余内容不变”等占位符；工具不会自动插入换行
- 每次写入成功后再生成下一块；全部写完后使用 \`read_file\` 分段抽查开头、衔接处和结尾
