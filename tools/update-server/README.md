...
`release.json` 中的 `version` 是所有插件客户端应安装的目标版本：

- 高于客户端版本时执行升级。
- 低于客户端版本时执行回退。
- 与客户端版本相同时不执行安装。

回退不需要增加额外配置。例如将已经安装 `3.1.0` 的客户端回退到 `3.0.16`：
...
```json
{
	...
	"mandatory": false,
	"releaseNotes": "回退到稳定版本。",
	"vsixPath": "dicode-3.0.16.vsix"
}
```

将 `mandatory` 设置为 `true` 后，客户端必须安装目标版本并重新加载窗口才能继续
使用 DiCode。关闭更新窗口、选择手动下载或重启旧版插件都不能绕过已经获取到的
强制安装要求。
